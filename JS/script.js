...
function displayUserInfo(user) {
    ...
    userInfoItem.className = 'nav-item user-info';
    userInfoItem.innerHTML = `
        <span class="nav-link">
            ${user.email} (${user.role})
            <button onclick="logout()" class="btn btn-sm btn-outline-danger ms-2">Logout</button>
        </span>
    `;
    headerNav.appendChild(userInfoItem);
}
...
async function handleContactForm(event) {
    ...
    try {
        const response = await fetch('https://api.web3forms.com/submit', {
            ...
            body: JSON.stringify({
                access_key: '7a878cff-514c-4db2-8d65-eb44b1b2229a', // Replace with your key
                ...formData
            })
        });
